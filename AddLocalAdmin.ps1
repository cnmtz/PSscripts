...
$rdpgroup = "Remote Desktop" #This will add the user to the RDP group but nnot grant them admin rights
$Domain     = "DOMAIN.LOCAL" 
...
switch ($result)
    {
        0 {
            ...
            Write-Host "$DomainUser has been added to the $admingroup on $Computer"
          }
        1 {
            [ADSI]"WinNT://$Computer/$rdpgroup,group").psbase.Invoke("Add",([ADSI]"WinNT://$Domain/$DomainUser").path)
            Write-Host "$DomainUser has been added to the $rdpgroup on $Computer"
          }
    }